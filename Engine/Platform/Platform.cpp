...
namespace andromeda::platform {
	...
	namespace {
			...
			get_from_handle(window_handle handle)
		{
				...
				resize_window(const window_info& info, const RECT& area)
			{
				...
				const s32 width{(s32)(window_rect.right - window_rect.left) };
				const s32 height{(s32)(window_rect.bottom - window_rect.top) };
		...
		create_window(const window_init_info* const init_info /*=nullptr*/) 
	{
		...
		wc.lpszClassName = L"AndromedaWindow";
