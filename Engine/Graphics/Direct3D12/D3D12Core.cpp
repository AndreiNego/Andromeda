...
namespace andromeda::graphics::d3d12::core {
	...
	namespace {
		...
		//This can be expanded into checking for screens, resolutions and multi-adapter settings.
		IDXGIAdapter4*
	...
	bool
		...
		//Enable debugging layer, Requires "Graphics Tools" optional feature
		{
			...
			{
				OutputDebugStringA("Warning: D3D12 Debug interface is not available. Verify that Graphics Tools optional feature is installed");
			}
		...
		bool result{ true };
		rtv_desc_heap.initialize(512, false);
		dsv_desc_heap.initialize(512, false);
		srv_desc_heap.initialize(4096, true);
		uav_desc_heap.initialize(512, false);
	...
	void shutdown()
	{
		...
		// NOTE: we don't call process_deferred_release at the end because some resources can't be released before their depending resources are released.
		//
		for (u32 i{ 0 }; i < frame_buffer_count; ++i)
		{
		...
		//NOTE: some types only use deferred release for their resource during shutdown/reset/clear
		//
		...
		render_surface(surface_id id)
	{
		...
		//done recording commands. execute commands now, signal and increment the fence value for the next frame.
